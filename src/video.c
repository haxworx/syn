...
void update_console(sound_t *sound)
{
    ...
    printf("BASS: %d MID: %d TREBLE: %d\n", global_bass, global_mid, global_treble);
...
void set_files_list(char *buf, int len)
{
    ...
    if (recording_end == 0) {
        snprintf(buf, len, "[ESC] (quit)  [DOWN/UP] (pitch -/+)  playback(\"%d\")  [DEL] (delete latest recording)", recording_start);

    } else {
        snprintf(buf, len, "[ESC] (quit)  [DOWN/UP] (pitch -/+)  playback(\"%d-%d\")  [DEL] (delete latest recording)", recording_start, recording_end);
...
void set_screen_data(sound_t * sound, char *buf, int len)
{
    ...
    case SYNTH_CREEPY_FUZZ:
        ...
        snprintf(buf, len, "Pitch: %0.2f Hz  Tone: %d  Volume: %d  Instrument: \"%s\"",
...
void update_screen(sound_t * sound)
{
    ...
    if (recording_enabled) {
        logo_copy = SDL_ConvertSurface(logo_recording, logo_recording->format, 0);
    } else {
    ...
    SDL_Surface *guide_text = TTF_RenderText_Blended(font, GUIDE_TEXT_0, whiteColor);
    ...
    SDL_Surface *action_text = TTF_RenderText_Blended(font, current_action_text, *dataColor);
    if (!action_text)
        fail("TTF_RenderText_Blended: %s\n", TTF_GetError());
    ...
    SDL_Surface *files_list_text = TTF_RenderText_Blended(font, files_list, whiteColor);
    if (!files_list_text) {
        fail("TTF_RenderText_Blended: %s\n", TTF_GetError());
    }
    

    SDL_Surface *version_text = TTF_RenderText_Blended(font, VERSION, whiteColor);  
    if (! version_text)
        fail("TTF_RenderText_Blended: %s\n", TTF_GetError());

    SDL_Surface *info_text = TTF_RenderText_Blended(font, INFO_TEXT, blueColor);
    if (!info_text)
        fail("TTF_RenderText_Blended: %s\n", TTF_GetError());
    ...
    action_rect.x = (WIDTH / 2) - (strlen(current_action_text) * (FONT_SIZE / 2)) / 2; 
    ...
    version_rect.x = WIDTH - ((2 * FONT_PADDING) + (FONT_SIZE / 2) * (strlen(VERSION)));    
