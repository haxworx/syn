...
void recording_start(void)
{
	...
	file_currently_in_use = strdup(filename);
        ...
        snprintf(path, sizeof(path), "%s%c%s", working_directory, SLASH, filename);
...
void recording_stop(void)
{
	...
	if (synth_continuous) {
        ...
        fwrite(&wav_file_hdr, sizeof(struct wav_file_hdr_t), 1,
               output_file);
	...
	free(file_currently_in_use); file_currently_in_use = NULL;
}
...
void waveform_default(void *sound, uint8_t * stream, int len)
{
	...
	if (snd->pitch > SOUND_PITCH_MAX) // ten times sanity
		...
		if (sound == 0) goto no_effects; /* audible clicks adios! */
                switch (snd->effects) {
                case SYNTH_DALEK:
...
void process_sound(sound_t * sound)
{
	...
	uint16_t pitch_old = sound->pitch;
        ...
        while (synth_continuous || continuous || (counter < 44100 * sound->duration)) {
                if ((1000 / FPS) > SDL_GetTicks() - start_time) {
                        SDL_Delay((1000 / FPS) -
			...
			if (keystate[scancode] == SDLK_ESCAPE) {
 			...
 			uint32_t mousestate =
				...
				if (!bending_pitch) bending_pitch = true;
                        } else if (mousestate &
	...
	if (bending_pitch) current_action("end of the bend...");
...
{
	if (file_currently_in_use != NULL && !strcmp(filename, file_currently_in_use)) {
		current_action("you cannot play a file that is currently recording");
		return;
	}
